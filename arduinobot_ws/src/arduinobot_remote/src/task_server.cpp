...
private:
  ...
  {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "Received goal request with task number: %d" << goal->task_number);
    (void)uuid;
  ...
  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduinobot_msgs::action::ArduinobotTask>> goal_handle)
  {
    ...
    if (!gripper_move_group_){
      gripper_move_group_ =
          std::make_shared<moveit::planning_interface::MoveGroupInterface>(shared_from_this(), "gripper");
    }
